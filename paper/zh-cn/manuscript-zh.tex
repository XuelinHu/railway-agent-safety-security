...
实验聚焦 ADE\cite{gurulingappa2012ade} 与 CoNLL04\cite{roth2004conll}。ADE 是含两类实体和一类关系的药物安全相关任务；CoNLL04 是含四类实体和五类有向关系的通用领域对照。两者均为英文句级输入，并分别训练与评估，因此本文不涉及跨数据集零样本迁移。主要贡献包括：
\begin{itemize}
...
\item 受控增强有效性与组件作用的实证证据。在两个基准上验证相对匹配纯源文本生成器的关系抽取增益，在两次追加训练及精确重叠排除后复核该增益，并通过消融刻画图谱上下文与接受门控对精确率和召回率的影响。
\end{itemize}
...
\section{结论与未来工作}
本文提出来源约束的图谱增强与证据门控接受机制，将候选生成和输出接受解耦。训练来源资格控制图谱提示的使用，源跨度重建与实体--关系检查展示保留输出的接受路径。统一严格跨度评估显示，PGE 相对 SOE 在 ADE 与 CoNLL04 上的关系 F1 分别提高 5.18 和 4.65 个百分点。排除 CoNLL04 精确训练--测试重叠后关系优势仍存在，两次追加训练也保持正差异。消融进一步区分了图谱条件候选恢复与选择性接受的作用，并指出端点保留中的精确率--覆盖率取舍。研究由此为面向人工复核的知识构建提供了受控、可审计的图谱增强抽取方法。
