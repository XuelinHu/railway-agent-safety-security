#!/usr/bin/env python3
"""Document-level paired inference for two promoted formal-test metrics."""
...
from bootstrap_compare import add_counts, compare_field  # noqa: E402
...
def load_metrics(path: Path) -> dict[str, Any]:
    ...
    if result.get("selection_split") == "validation" or result.get("formal_test_read") is not True:
        raise ValueError(f"formal-test metric proof is missing: {path}")
    return result


def document_units(left: dict[str, Any], right: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for unit_id in sorted(left_jobs):
        ...
        unit = units.setdefault(
            document_id,
            {
                ...
                "left": {field: {"gold": 0, "predicted": 0, "correct": 0} for field in FIELDS},
                "right": {field: {"gold": 0, "predicted": 0, "correct": 0} for field in FIELDS},
            },
        )
...
def run(args: argparse.Namespace) -> int:
    ...
    if left.get("selection_split") != right.get("selection_split"):
        raise ValueError("formal-test metric selection splits differ")
    units = document_units(left, right)
    rng = random.Random(args.seed)
    result = {
        "metric": "strict-global-character-span-one-to-one",
        "selection_split": left["selection_split"],
