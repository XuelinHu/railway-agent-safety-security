# Paper metadata

This file preserves the author information supplied for the planned manuscript. Verify the author-affiliation mapping and corresponding-author designation before submission.

## Target venue and working title

- Journal: *Journal of Safety Science and Resilience*
- Special issue: *Agent for Safety and Security: Complex Network Modeling, Substructure Analysis, Risk Discovery, and Resilience Assessment*
- Submission deadline: 28 February 2027
- Working title: *Provenance-Preserving Evidence-Gated Knowledge-Graph Augmentation for Auditable Entity–Relation Extraction*
- Current manuscript scope: ADE and CoNLL04 only; see `paper/README.md` for the active delivery.
- Review model: double blind; upload the anonymous manuscript and title page separately.

## Authors

1. Xuelin Hu
2. Xiaoqin Fu
3. Youjing Fu
4. Jingchao Wang
5. Ruishen Liu
6. Charles Jumaa Katila

## Affiliations and contacts

### Liuzhou Railway Vocational Technical College

No. 2 Wenyuan Road, Yufeng District, Liuzhou City, Guangxi Zhuang Autonomous Region, 545000, China.

- Xuelin Hu
  - ORCID: 0000-0002-4475-3034
  - Email: huxuelinai@gmail.com; huxl@ltzy.edu.cn
- Xiaoqin Fu
  - ORCID: 0009-0003-5123-8393
  - Email: xiaoqin.fu@qq.com; fuxiaoqin@ltzy.edu.cn

### College of Earth and Environmental Sciences, Lanzhou University

Lanzhou 730000, China.

- Youjing Fu
  - ORCID: 0009-0004-1369-7879
  - Email: fuyj2025@lzu.edu.cn

### School of Computer Science, Zhongyuan University of Technology

Zhengzhou 450007, China.

- Jingchao Wang
  - Corresponding author
  - Email: static@zut.edu.cn

### School of Artificial Intelligence, Luoyang Normal University

Luoyang 471000, China.

- Ruishen Liu
  - Email: liuruishen@lynu.edu.cn

### School of Computing and Mathematics, The Co-operative University of Kenya

- Charles Jumaa Katila
  - Email: ckatila@cuk.ac.ke

## Acknowledgement

This work was supported by Science and Technology Research Project of Henan Province (grant no. 262102211055).
